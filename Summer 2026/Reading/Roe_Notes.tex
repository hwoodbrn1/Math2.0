\documentclass[12pt, reqno]{extarticle}
...
\section{Connections, Curvature, and Characteristic Classes}

Let $G$ be a Lie group. 

\begin{definition}
    We define a \textit{principal bundle} $E$ with \textit{structural group} $G$ over a smooth 
    manifold $M$ to be a locally trivial fiber bundle whose fibers are homeomorphic to $G$,
    considered as a right $G$-space. Therefore, $G$ acts smoothly on $E$ by right multiplication on 
    each fiber, and $M = E/G$. 
\end{definition}

Examples of these are frame bundles of vector bundles. These are principal $\textrm{GL}(k)$-bundles.

On the other hand, given a principal bundle, there is a way to construct a vector bundle 
with a $G$ action, called the \textit{associated vector bundle} to $E$. Specifically,
given a representation $\rho: G \to \text{GL}(F)$ of $G$ on a vector space $F$, 
$G$ operates on the space $E \times F$ by
\[
    (e,f)g = (eg, \rho(g^{-1})f).
\]
We define the associated vector bundle as the quotient space $E \times_\rho F$ under the $G$ action. 
It is indeed a vector bundle whose fibers are isomorphic to $F$.

Now we will make some definitions about differential forms on vector bundles. Let $E$ be a principal bundle
with group $G$. To any $u \in \mathfrak{g}$, the Lie algebra of $G$, we can associate a $G$-invariant
vector field on $E$. This is called the Killing Field. 

\begin{definition}
    Let $p \in M$ and choose $s \in E_p$. Consider the map $m_s: G \to E_p$ given by 
    $g \mapsto sg$. This is in fact a diffeomorphism, so that for every $g \in G$, the 
    differential $dm_s|_g: T_gG \to T_{gs}E_s$ is an isomorphism. Taking $g = e \in G$, we
    obtain an isomorphism $\phi_p:njbmT_eG = \mathfrak{g} \to T_s E_p$. For $u \in \mathfrak{g}$,
    we define the \textit{Killing field} $X_u$ to be the vector field on $E$ whose 
    value at a point $s \in E_p$ is $\phi_p$
\end{definition}
...
\section{Analytic Properties of Dirac Operators}

This section covers Sobolev spaces, continuity results for (generalized) Dirac operators, 
and the functional calculus. 

\subsection{Sobolev spaces}

We work on the $n$-torus $\TT^n$. We will define the sobolev spaces $H^k(\TT^n)$ for $k$ any positive 
integer using a version of the Fourier transform. Let $f: \TT^n \to \CC$ be an integrable function.
Define its Fourier transform $\widehat{f}: \ZZ^n \to \CC$ as 
\[
    \widehat{f}(\nu) = \frac{1}{{(2\pi)}^n}\int_{\TT^n}f(x)e^{i\langle x, \nu\rangle}dx.
\]
...
\begin{theorem}[Parseval's Theorem]
    The Fourier Transform $\mathcal{F}$ is an isometry $\mathcal{F}: L^2(\TT^n) \to L^2(\ZZ^n)$,
    where the second space is equipped with the (normalized) counting measure. In other words,
    \[
        \int_{\TT^n}|f|^2 = (2\pi)^n\sum_{\nu}|\widehat{f}(\nu)|^2.
    \]
\end{theorem}

\begin{theorem}[Fourier Inversion Theorem for $L^2$]
    For $f \in L^2(\TT^n)$, the Fourier series of $f$ converges in the $L^2$ norm.
\end{theorem}

This gives us a way to translate from functions on $\ZZ^n$ back to functions on $\TT^n$. 

\begin{theorem}[Fourier Inversion Theorem for $C^\infty$]
    For $f \in C^\infty(\TT^n)$, the Fourier series of $f$ converges in the Frechet $C^\infty$
    topology to $f$. The Fourier coefficients $\widehat{f}(\nu)$ are rapidly decreasing.
\end{theorem}

\begin{definition}
    Let $k$ be a positive integer. Define the \textit{Sobolev $k$-inner product} on $C^\infty(\TT^n)$
    by the formula
    \[
        \langle f_1, f_2\rangle_k = (2\pi)^n \sum_{\nu}\widehat{f_1}(\nu)\overline{\widehat{f_2}(\nu)}
        (1 + |\nu|^2)^k.
    \]
    This makes sense since both $\widehat{f_1}$ and $\widehat{f_2}$ are rapidly decreasing. 
    The norm induced by this inner product is called the \textit{Sobolev $k$-norm}.

    We define the \textit{$k$-th Sobolev Space} $H^k(\TT^n)$ to be the completion of 
    $C^\infty(\TT^n)$ in the Sobolev $k$-norm.
\end{definition}

\begin{proposition}
    There is a continuous inclusion $C^\infty(\TT^n) \to W^k(\TT^n)$ for all $k$.
\end{proposition}

\begin{theorem}[Sobolev Embedding Theorem]
    For any integer $p > n/2$, there is a continuous embedding $W^{k + p} \to C^k$.
\end{theorem}

\begin{theorem}[Rellich's Theorem]\label{rellich}
    If $k_1 < k_2$, there is a compact inclusion $W^{k_2} \to W^{k_1}$.
\end{theorem}

We define Sobolev spaces on manifolds using partitions of unity. While the Sobolev $k$-norm will
depend on the chosen atlas, one can show the induced topology is independent of this choice.

\subsection{Analysis of the Dirac Operator}

Recall the Weitzenbock formula,
\[
    D^2 = \nabla^*\nabla + \mathcal{K},
\]
where $\mathcal{K}$ is a certain curvature operator. In fact, we can define
\textit{generalized} Dirac operators to be operators $D$ such that
\[
    D^2 = \nabla^*\nabla + B,
\]
...
\[
    \|Ds\|_0 \leq C\|s\|_1,
\]
since $D$ is a first order operator. For generalized Dirac operators, we have an 
``approximate converse'':

\begin{theorem}[Garding's Inequality]
    Let $D$ be a generalized Dirac operator on a compact manifold. Then there is a constant $C$
    such that for any $s \in C^\infty(S)$, 
    \[
...
\begin{proposition}
    For any integer $k > 0$ there is a constant $C_k$ such that for any $s \in C^\infty(S)$,
    \[
...
\begin{proposition}
    The closure $\overline{G}$ of the graph $G$ of the Dirac operator is also the graph of an operator,
    which we will denote by $\overline{D}$.
\end{proposition}

The proof of this fact relies on the existence of a formal adjoint of $D$. The domain of $\widehat{D}$
consists of all $x \in L^2(S)$ such that there is a sequence $x_j$ converging to $x$, and with $Dx_j$ 
converging in $L^2(S)$ as well. By applying Garding's inequality, we get
\[
    \|x_i - x_j\|_1 \leq C(\|x_i - x_j\|_0 + \|Dx_i - Dx_j\|_0) \to 0,
\]
and thus the sequence $x_j$ is cauchy in $\|\cdot\|_1$ and converges to some $x'$. Then since the $L^2$
topology is weaker than the $H^1$ topology, this implies $x_j \to x'$ in $L^2$, and thus $x' = x$ by
uniqueness of limits. Thus $x \in H^1$. 

Conversely, if $x \in H^1$, take some smooth $x_j$ converging to $x$ in $H^1$. Then since $D$
is continuous $H^1 \to H^0$, 
\[
    \|Dx_i - Dx_j\|_0 \leq \|x_i - x_j\|_1 \to 0,
\]
so that $Dx_j$ converges in $L^2$. And $x_j$ clearly converges to $x$ in $L^2$ as well, 
so $x \in \text{Dom}(\overline{D})$.

\begin{proposition}
    Suppose $x, y \in L^2(S)$ with $Dx = y$ weakly, i.e. 
    \[
...
\textbf{Proof:} To prove this, the book uses a Friedrichs' mollifier. We obtain 
some bounded sequence $x_\epsilon \to x$ weakly as $\epsilon \to 0$, and prove that also $Dx_\epsilon$
is bounded. Applying Garding's inequality, we see that $x_\epsilon$ is bounded in the $H^1$ norm, and
thus converges in $L^2$ to $x$ by Rellich's theorem \ref{rellich}. This proves $x \in H^1$.

Then $\overline{D}x = z$ for some $z$. To prove that $z = x$, it is enough show that $z$ is a weak
solution to $D$ as well and then apply uniqueness of weak derivatives.\qed

\begin{proposition}
    The Kernel of $D$ consists of smooth sections. 
\end{proposition}

\textbf{Proof:} We will prove inductively that $s \in H^k$ for all $k$, and the result will follow by the 
Sobolev embedding theorem. We have 
\[
    \|s_\epsilon\|_k \leq C_k(\|s_\epsilon\|_{k-1} + \|Ds_\epsilon\|_{k-1}) 
    = C_k(\|s_\epsilon\|_{k-1} + \|[D,F_\epsilon]s\|_{k-1}),
\]
since $Ds = 0$. But $F_\epsilon$ and $[D,F_\epsilon]$ form bounded families of operators. Then
$s_\epsilon$ is a bounded sequence in $H^k$ which converges in $L^2$ to $s$, and must converge weakly in 
$H^k$. Then $s \in H^k$ as desired. 

\subsection{The Functional Calculus}

We will not prove the next theorem, but it is some version of a spectral theorem for unbounded
self adjoint operators. 

\begin{theorem}
    There is a direct sum decomposition of $H$ into a sum of countably many finite dimensional
    eigenspaces $H_\lambda$, each consisting of smooth sections with eigenvalue $\lambda$. 
    These eigenvalues form a discrete subset of $\RR$.
\end{theorem}
...
\begin{definition}
    A bounded operator $A$ on $L^2(S)$ is called a \textit{smoothing operator} if there is
    a smooth \textit{kernel} $k(p,q)$ on $M\times M$ with values in $Hom(S_q, S_p)$, such
    that 
    \[
