...
\begin{enumerate}
    ...
    \item[5.] Let $R$ be a ring and $G$ be an infinite multiplicative cyclic group with generator $\xi$.
    Then $R[G]$ is not isomorphic to the polynomial ring $R[x]$. This is because the group of units of $R[x]$ 
    consists of only the units of $R$, as the product of two nonconstant polynomials will have degree
    equal to the sum of degrees of each polynomial, and therefore cannot be one. Then the only 
    elements which can be units are the constant polynomials given by elements of $R$. 

    But the units of $R[G]$ include both those in $R$ and the elements $\xi^n$ for $n \in Z$, as well 
    as others. This is not isomorphic to just the group of units of $R$. Then $R[G]$ cannot be isomorphic 
    to $R[x]$, as their groups of units would have to be isomorphic as well. 
