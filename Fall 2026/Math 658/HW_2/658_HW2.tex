...
\begin{enumerate}
    ...
    \item[2.3.5.] (a.) First, we can see that the quotient converges pointwise to
    ...
    \[
        ...
        \leq h\epsilon/2
    \]
    for $x \in K^c$, given that $t$ is small enough so $x + te_i$ also lies within $K^c$. 

    Moreover, within $K$, pointwise convergence implies uniform convergence, so we can choose $h$ sufficiently small so that
    the quotient is as close to $\partial_i f$ as we want. 
