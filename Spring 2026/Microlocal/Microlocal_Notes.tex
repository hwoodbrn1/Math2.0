...
\subsection{Section 4. Exercises}

\begin{enumerate}

    \item[1.] (a.) We had to show the Sobolev $H^s(\RR^n)$ norm could be defined using either of the formulas 
    \[
        \|u\|_{H^s} = \frac{1}{{(2\pi)}^n}\|\langle \xi\rangle^s \hat{u}\|_{L^2}
    \]
    \[
        \|u\|_{H^s} = \sum_{|\alpha| \leq s} \|D^\alpha u\|_{L^2}.
    \]

    (b.) This exercise asks us to show the linear map $\RR^n \to \RR^{n-d}$ sending a
    schwartz function $u$ to its restriction $\tilde{u} = u|_{x'' = 0}$ is continuous
    $H^s(\RR^n) \to H^{s-d/2}(\RR^{n-d})$.
    
    Using the Fourier Inversion formula, we can write
    \[
        u(x', 0) = \int_{\RR^{n-d}}\int_{\RR^d} e^{ix'\cdot \xi'} \hat{u}(\xi', \xi'')d\xi''d\xi' = 
        \mathcal{F}_{x'}^{-1}\left(\int_{\RR^d}\hat{u}(\xi',\xi'')d\xi''\right).
    \]
    Then after taking the fourier transform of the restricted function, we have
    \[
        \mathcal{F}_{x'}\left(\tilde{u}\right) = \int_{\RR^d}\hat{u}(\xi',\xi'')d\xi''.
    \]
    Now we can write
    \[
        \|\langle \xi\rangle^{s-d/2}\hat{\tilde{u}}\|_{L^2(\RR^{n-d})}
        = \|\|\langle \xi\rangle^{s-d/2}{\left|\int_{\RR^d}\hat{\tilde{u}}(\xi',\xi'')d\xi''\right|}^2\|_{L^2(\RR^{n-d})}
        \leq \|\langle \xi\rangle^{s-d/2}\int_{\RR^d}|\hat{\tilde{u}}(\xi',\xi'')d\xi''\|_{L^2(\RR^{n-d})}.
    \]
